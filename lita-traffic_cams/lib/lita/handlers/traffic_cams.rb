  ...
  module Handlers
    class TrafficCams < Handler
      ...
      route(/where are all of the (available\s)?traffic cams[?]?/i, command: false, help: {"Where are all of the traffic cams?" => "Displays a map of the available traffic cams"}) do |response|
        response.reply "http://tinyurl.com/n39lkx2"
      end

      def traffic_at_city_hall(response)
        location_data = get_location_at(response, "city hall")

        # return response.reply("#{location_data.to_yaml} at #{location}")
      ...
      def traffic_at(response)
        response.reply("#{response.matches.to_json}\n\n")
        location = response.matches[0][4]

        location_data = get_location_at(response, location)

        # return response.reply("#{location_data.to_yaml} at #{location}")
        return response.reply("You know what? I can't figure out where \"#{location}\" is, but how about a selfie instead?") if location_data.nil? or location_data.empty?
        location_data['url'].each do |url|
          response.reply(url)
        end unless location_data.nil?
      end
      ...
      def get_location_at(response, location)
        ...
        response.reply "Looks like you're at #yychackathon, the traffic looks alright to me but here are the closest traffic cams" if location.eql? 'city hall'
